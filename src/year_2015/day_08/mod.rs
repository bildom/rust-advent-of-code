...
puzzle_solver!(
    [2015, 8] = {
        fn solve(&mut self, input: &str) -> anyhow::Result<Answer> {
            let mut raw_length = 0u16;
            let mut parsed_length = 0u16;
            let mut encoded_length = 0u16;

            for line in input.lines() {
                raw_length += line.len() as u16;
                parsed_length += StringParser.get_unescaped_string_as_u8(line)?.len() as u16;
                encoded_length += format!("\"{}\"", line.escape_default()).len() as u16;
            }
